...
const ChatDrawer: React.FC<ChatDrawerProps> = ({ title, recipientName, channel = 'inapp', type = 'health', onClose }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const filtered = res.notifications.filter((n: any) => n.channel === channel && (n.title.includes(recipientName) || n.message.includes(recipientName) || n.type === type));
      setMessages(filtered.slice(0, 20));
    } catch {}
  };
  useEffect(() => { void load(); }, [recipientName, channel, type]);

  const handleSend = async () => {
    if (!input.trim() || input.trim().length < 2) return;
    setSending(true);
    try {
      await sendNotification({ title: `${title} → ${recipientName}`, message: input.trim(), type: type as any, channel });
      setMessages(prev => [{ id: `tmp-${Date.now()}`, title: `You → ${recipientName}`, message: input.trim(), sentAt: new Date().toISOString(), type, channel }, ...prev]);
      setInput('');
    } catch {}
    setSending(false);
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="glass-card animate-slide-up" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px', width: '92%', height: '70vh', display: 'flex', flexDirection: 'column', padding: '1rem', position: 'fixed', bottom: '1rem', right: '1rem', maxHeight: '70vh' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)', paddingBottom: '0.6rem' }}>
          <h3 style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}><MessageCircle size={18} /> {title} — {recipientName}</h3>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0.8rem 0', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          {messages.length === 0 ? (
            <div style={{ textAlign: 'center', opacity: 0.6, padding: '1rem' }}>
              ...
              <p style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>No messages yet — start the conversation. Stored in PostgreSQL <code>notifications</code> `channel {channel}` type {type}.</p>
            </div>
          ) : messages.map((m: any) => (
        ...
        <div style={{ display: 'flex', gap: '0.5rem', borderTop: '1px solid var(--border)', paddingTop: '0.6rem' }}>
          <input aria-label="Message" placeholder={`Message to ${recipientName}...`} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} style={{ flex: 1, padding: '0.6rem', borderRadius: '8px', border: '1px solid var(--border)' }} maxLength={300} />
        ...
        <span className="hint-text" style={{ fontSize: '0.7rem', textAlign: 'center', marginTop: '0.4rem' }}>Via <code>POST /notification/send</code> → PostgreSQL `notifications` → Inbox</span>
      </div>
    </div>
  );
};
