...
import { getConfig, ok, fail } from '@nutrivedha/shared';
...
// ---------------------------------------------------------------
// POST /api/ai/scan
// Simulates computer-vision analysis of an uploaded image.
// Uses Gemini when available, otherwise deterministic mock.
// ---------------------------------------------------------------
...
router.post('/scan', async (req: Request, res: Response) => {
  ...
  try { result = JSON.parse(text); } catch { result = fallback; }
...
// ---------------------------------------------------------------
// POST /api/ai/diet
// Generates a 3-meal Ayurvedic diet plan based on a condition.
// ---------------------------------------------------------------
interface Meal {
  time: string;
  meal: string;
...
router.post('/diet', async (req: Request, res: Response) => {
  ...
  const { text, usedLive } = await generate(prompt, fallbackText);
  let meals: Meal[];
  try { meals = JSON.parse(text); } catch { meals = plan; }
...
// ---------------------------------------------------------------
// POST /api/ai/recipes
// Matches kitchen ingredients to Ayurvedic recipes (or asks Gemini).
// ---------------------------------------------------------------
const RECIPE_BANK = [
  {
...
router.post('/recipes', async (req: Request, res: Response) => {
  ...
  const { text, usedLive } = await generate(prompt, fallbackText);
  let recipes: unknown[];
  try { recipes = JSON.parse(text); } catch { recipes = matched.length ? matched : RECIPE_BANK.slice(0, 2); }
...
// ---------------------------------------------------------------
// POST /api/ai/chat
// Chatbot knowledge-base responses.
// ---------------------------------------------------------------
